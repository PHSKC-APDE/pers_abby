...
brfss_updated <- brfss %>% 
  filter(!Indicator %in% c("smoker1","firearm4","obese","_pastaer", "mjpast30")) %>% 
...
rename("indicator_key" ="Indicator", 
        ...
        "run_date" = "runid") %>% 
  select(data_source, indicator_key, tab, year, cat1, cat1_group, cat1_group_alias, cat1_varname, cat2, cat2_group, cat2_group_alias, cat2_varname, result, lower_bound, upper_bound, se, rse, comparison_with_kc, time_trends, significance, caution, everything()) %>% 
  bind_rows(., brfss_new) 
...
brfss_meta_updated <- brfss_metadata %>% 
  ...
  mutate(valence = case_when(
           varname == "firearm4" ~ "neutral",
           varname == "_pastaer_v1" ~ "positive",
           varname == "_pastaer_v2" ~ "positive",
           TRUE ~ "negative"),
         map_type = "HRA",
         ...
         unit = "adults",
         chi = ifelse(Indicator == "_pastaer_v2", 0, 1)) %>% 
  rename("indicator_key" = "Indicator",
         "short_name" = "title",
         "result_type" = "ratetype",
         "latest_year_result" = "latest_rate",
         "run_date" = "runid") %>% 
  select(-latest_data, -trendsummary, -varname, -Topic_new, -Domain_new, -savname, -sav_abbrev, -task_id) %>% 
  select (indicator_key, short_name, result_type, valence, latest_year, latest_year_result, latest_year_kc_pop, latest_year_count, map_type, unit, valid_years, chi, run_date)
 
list_of_datasets <- list("results" = brfss_updated, "metadata" = brfss_meta_updated)
write.xlsx(list_of_datasets, file = "//Phshare01/epe_share/WORK/CHI Visualizations/BRFSS Indicators (all)/BRFSS (all)/BRFSS_combined_tableau_suppressed_041819.xlsx")
